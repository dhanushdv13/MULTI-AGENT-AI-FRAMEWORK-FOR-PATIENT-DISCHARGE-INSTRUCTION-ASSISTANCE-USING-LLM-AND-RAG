...
from app.ai.agents.base import BaseAgent, format_search_results
...
Format meal plans clearly with times and portions."""
    ...
    async def process_async(self, query: str) -> str:
        """
        Process a diet-related query (Async).
        """
        # Search discharge for medical conditions
        discharge_results = await self.search_discharge_async("diagnosis condition disease medication", k=5)
        
        # Search dietary guidelines
        diet_results = await self.search_dietary_async(query, k=5)
        
        discharge_context = format_search_results(discharge_results) if discharge_results else "No relevant medical information found in this document."
...
## Response:"""
        ...
        return await self.ask_llm_async(prompt)
...
## Response:"""
    ...
    def generate_meal_plan(self, days: int = 7) -> str:
        ...
        discharge_context = format_search_results(discharge_results) if discharge_results else "No medical conditions found in this document."
...
Since I found no medical details in this document, here is general advice:
