...
async def get_extracted_text(doc_id: str, user: dict = Depends(get_current_user)):
    """Return extracted content — serves .docx if available, falls back to plain text."""
    doc = await _svc.get_one(doc_id, str(user["_id"]))

    # Prefer DOCX (PDF uploads)
    docx_path = doc.get("extracted_docx_path")
    ...
    # Fallback: plain text (image uploads)
