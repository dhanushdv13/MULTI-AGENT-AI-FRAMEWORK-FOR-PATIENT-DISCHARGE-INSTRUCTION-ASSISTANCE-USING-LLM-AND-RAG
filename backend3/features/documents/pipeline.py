...
from json_to_word import pdf_to_word as _pdf_to_word
...
class DocumentPipeline:
    ...
    def _process(self, file_path: str, vector_id: str, doc_id: str) -> Tuple[int, str, str, Optional[str]]:
        """
        Synchronous heavy work (runs in threadpool):
          1. Extract text (PDF or image)
          2. Save OCR cache + extracted text
          3. Generate .docx from OCR cache (PDF only)
          4. Smart chunk
          5. Build + save FAISS vectorstore
        Returns: (page_count, ocr_cache_path, extracted_text_path, docx_path)
        """
        ...
        # 3. Generate .docx from OCR cache (PDF only — images use txt fallback)
        docx_path: Optional[str] = None
        if ext not in IMAGE_EXTENSIONS:
            try:
                docx_out = str(extracted_dir / f"{stem}_extracted.docx")
                _pdf_to_word(
                    input_pdf=file_path,
                ...
                docx_path = docx_out
                logger.info(f"DOCX generated: {docx_out}")
            except Exception as e:
                logger.warning(f"DOCX generation failed (will fall back to txt): {e}")
        ...
        # 5. Convert to LangChain Documents + build FAISS
        lc_docs = [
            Document(
                page_content=chunk,
        ...
        return page_count, ocr_cache_path, extracted_text_path, docx_path
    ...
    async def run(self, doc_id: str, user_id: str, file_path: str, vector_id: str):
        try:
            ...
            page_count, ocr_cache_path, extracted_text_path, docx_path = await run_in_threadpool(
