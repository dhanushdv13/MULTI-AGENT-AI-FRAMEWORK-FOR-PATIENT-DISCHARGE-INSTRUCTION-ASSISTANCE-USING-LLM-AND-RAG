...
Uses OCR cache from pipeline.py when available (no double OCR).
...
logger = logging.getLogger("json_to_word")


# ══════════════════════════════════════════════════════════════════════
# OCR Cache — reads results saved by pipeline.py
# ══════════════════════════════════════════════════════════════════════

def _load_ocr_cache(pdf_path: str, json_path: str = None) -> list | None:
...
def pdf_to_word(input_pdf, output_docx=None, dpi=150, max_workers=8, ocr_json_path=None):
    """
    Convert PDF to Word — FAST.

    Uses OCR cache when available (from pipeline.py).
    ...
    # ── Step 1: Check for OCR cache (from pipeline) ──────────────
    cache = _load_ocr_cache(str(input_path), ocr_json_path)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python json_to_word.py <input.pdf> [output.docx] [ocr_cache.json]")
        sys.exit(1)

    pdf_in = sys.argv[1]
    docx_out = sys.argv[2] if len(sys.argv) >= 3 else None
    json_in = sys.argv[3] if len(sys.argv) >= 4 else None
    output = pdf_to_word(pdf_in, docx_out, dpi=150, max_workers=8, ocr_json_path=json_in)
